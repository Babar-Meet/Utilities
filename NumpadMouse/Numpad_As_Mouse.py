...
# Thread-safe shift state tracking
...
def scroll_mouse(direction, speed):
    if direction == 'up':
...
def mouse_control_loop():
    while True:
        ...
        # Get current shift state safely
        with shift_lock:
            current_shift_state = shift_pressed
            
        # Speed priority: Caps Lock (slow) > Shift (fast) > Normal
        if is_caps_lock_on():
            current_speed = caps_lock_speed
            current_scroll_speed = normal_scroll_speed  # Caps lock doesn't affect scroll speed
        else:
            current_speed = fast_speed if current_shift_state else normal_speed
            current_scroll_speed = fast_scroll_speed if current_shift_state else normal_scroll_speed
...
# Register global toggle hotkey (F9)
